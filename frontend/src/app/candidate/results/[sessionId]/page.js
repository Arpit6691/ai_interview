...
export default function CandidateResultsPage() {
  ...
  if (!data) {
    return (
      <div className="min-h-screen bg-[#090d16] flex items-center justify-center flex-col gap-4 text-white">
        <h2 className="text-xl">Evaluation Not Found</h2>
        <button onClick={() => router.push('/candidate/dashboard')} className="text-indigo-400">Back to Dashboard</button>
      </div>
    );
  }

  const { session, evaluation, transcripts, suspiciousActivities } = data;
